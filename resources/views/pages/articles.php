...
// Убедимся что переменная $posts определена
$posts = $posts ?? [];
?>
        ...
        <div class="articles-stats">
            ...
            <div class="stat-item">
                <div class="stat-number">
                    <?php
                    if (!empty($posts)) {
                        // Получаем дату первой статьи
                        $firstDate = null;
                        foreach ($posts as $post) {
                            if (!empty($post['meta']['date'])) {
                                $date = $post['meta']['date'];
                        ...
                        echo $firstDate ? date('Y', strtotime($firstDate)) : '—';
                    } else {
                        echo '—';
                    }
                    ?>
            ...
            <div class="stat-item">
                <div class="stat-number">
                    <?php
                    if (!empty($posts)) {
                        // Получаем дату последней статьи
                        $lastDate = null;
                        foreach ($posts as $post) {
                            if (!empty($post['meta']['date'])) {
                                $date = $post['meta']['date'];
                        ...
                        echo $lastDate ? date('Y', strtotime($lastDate)) : '—';
                    } else {
                        echo '—';
                    }
                    ?>
        ...
        <div class="articles-list">
            <?php if (!empty($posts)): ?>
                <?php foreach ($posts as $index => $post):
                    // Извлекаем метаданные из YAML фронт-меттера
                    $meta = $post['meta'] ?? [];

                    // Если метаданные не извлечены, пытаемся извлечь из контента
                    if (empty($meta) && !empty($post['content'])) {
                    ...
                    // Удаляем YAML фронт-меттер для создания excerpt
                    $contentWithoutYaml = preg_replace('/^---\s*\n.*?\n---/s', '', $content);
                    $excerpt = strip_tags($contentWithoutYaml);
                    $excerpt = trim($excerpt);
                    $excerpt = strlen($excerpt) > 150 ? substr($excerpt, 0, 150) . '...' : $excerpt;

                    $readTime = $post['read_time'] ?? 3; // Используем значение из контроллера

                    // Если нет времени чтения, рассчитываем его
                    if (!isset($post['read_time'])) {
                        $wordCount = str_word_count(strip_tags($contentWithoutYaml));
                        $readTime = max(1, ceil($wordCount / 200)); // минимум 1 минута
                    }
                    ...
                    <article class="article-card" data-aos="fade-up" data-aos-delay="<?= $index * 100 ?>">
                        ...
                        <div class="article-tags">
                            <?php if (!empty($meta['tags'])): ?>
                                <?php
                                $tags = is_array($meta['tags']) ? $meta['tags'] : explode(',', $meta['tags']);
            ...
            <?php else: ?>
                <div class="no-articles">
                    <div class="empty-state">
                        ...
                        <?php if (class_exists('\App\Core\Auth') && \App\Core\Auth::check()): ?>
